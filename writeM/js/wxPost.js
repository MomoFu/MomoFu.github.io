    ...
    $.getJSON('//wx.chubao.cn/getwxconf?url='+myUrl+'&callback=?', function(remoteData){
	 ...
	 wx.ready(function(){
	  wx.onMenuShareTimeline({
		title: me, // 分享标题
		link: 'http://marketing1.chubao.cn/writeM/index.html?u='+u,
		imgUrl: imgLink, // 分享图标，需替换为图片地址
		success: function () {
			
			_hmt.push(['_trackEvent', "guessWhat", "guessWhatResultShareToTimeLineSuccess"]);
		},
		cancel: function () {
	  ...
	  wx.onMenuShareAppMessage({
		...
		success: function () {
			 ...
			 _hmt.push(['_trackEvent', "guessWhat", "guessWhatResultShareToFriendSuccess"]);
		},
		cancel: function () {
			// 用户取消分享后执行的回调函数，需要时选填
		}
